...
@section('title', 'Edit Catatan Pembelian Eksternal')
...
@section('content')
    ...
    <nav class="flex items-center text-sm font-medium mb-6">
        ...
        <span class="text-slate-900 dark:text-white">Edit Catatan</span>
    </nav>

    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
        <div class="p-6 border-b border-slate-100 dark:border-slate-800">
            <h2 class="text-lg font-bold text-slate-900 dark:text-white">Formulir Edit Pembelian Eksternal</h2>
        </div>

        <form action="{{ route('external-purchases.update', $externalPurchase->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col">
                    ...
                    <input type="date" id="purchase_date" name="purchase_date"
                        ...
                        value="{{ old('purchase_date', $externalPurchase->purchase_date->format('Y-m-d')) }}" required>
                    @error('purchase_date')
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" id="item_name" name="item_name"
                        class="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 dark:focus:ring-primary/50 text-slate-900 dark:text-white"
                        value="{{ old('item_name', $externalPurchase->item_name) }}" placeholder="Contoh: Tinta Printer Epson" required>
                    @error('item_name')
                        <p class="text-sm text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" id="source_shop" name="source_shop"
                        class="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 dark:focus:ring-primary/50 text-slate-900 dark:text-white"
                        value="{{ old('source_shop', $externalPurchase->source_shop) }}" placeholder="Contoh: Toko ATK Jaya" required>
                    @error('source_shop')
                        <p class="text-sm text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex flex-col">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="price" name="price"
                            ...
                            value="{{ old('price', $externalPurchase->price) }}" placeholder="150000" min="0" required>
                    </div>
                     @error('price')
                        <p class="text-sm text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex flex-col">
                    ...
                    <select id="payment_status" name="payment_status"
                        class="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 dark:focus:ring-primary/50 text-slate-900 dark:text-white"
                        required>
                        <option value="belum lunas" {{ old('payment_status', $externalPurchase->payment_status) == 'belum lunas' ? 'selected' : '' }}>Belum Lunas</option>
                        <option value="lunas" {{ old('payment_status', $externalPurchase->payment_status) == 'lunas' ? 'selected' : '' }}>Lunas</option>
                    </select>
                ...
                <div class="flex flex-col md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                        class="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 dark:focus:ring-primary/50 text-slate-900 dark:text-white"
                        placeholder="Informasi tambahan seperti detail barang, nomor referensi, dll.">{{ old('notes', $externalPurchase->notes) }}</textarea>
                    @error('notes')
                         <p class="text-sm text-rose-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
