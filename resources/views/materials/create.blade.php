...
@section('content')
    ...
    <nav class="flex items-center text-sm font-medium mb-6">
        ...
        <span class="material-symbols-outlined text-slate-300 mx-2 text-base">chevron_right</span>
        <span class="text-slate-900 dark:text-white">Tambah Bahan</span>
    </nav>

    <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6">
        <h2 class="text-lg font-bold text-slate-900 dark:text-white mb-6">Tambah Bahan Baru</h2>
        ...
        <form action="{{ route('materials.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    <label for="current_stock" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Stok Awal</label>
                    <input type="number" id="current_stock" name="current_stock" min="0" step="0.01" required
                        class="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary dark:bg-slate-700 dark:text-white"
