            ...
            x-data="{
                ...
                fetchCategories() {
                    ...
                    if (this.searchTerm.length > 0 && this.searchTerm.length < 3) {
                        this.categoriesHtml = '<div class="p-6 text-center text-slate-500 dark:text-slate-400">Masukkan minimal 3 karakter untuk mencari.</div>';
                        this.loading = false;
                        return;
                    }

                    fetch(`{{ route('categories.index') }}?search=${this.searchTerm}`, {
                        headers: {
                    ...
                    .catch(error => {
                        ...
                        this.loading = false;
                        this.hasError = true;
                        this.errorMessage = 'Gagal memuat kategori. Silakan coba lagi.';
                    });
                },
                init() {
                    this.fetchCategories();
                    this.$watch('categoriesHtml', () => {
                        this.$nextTick(() => {
                ...
                handlePaginationClick(event) {
                    ...
                    .catch(error => {
                        ...
                        this.loading = false;
                        this.hasError = true;
                        this.errorMessage = 'Gagal memuat halaman kategori. Silakan coba lagi.';
                    });
                }
            }"
        >
            <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between">
                ...
                <div class="flex items-center gap-2 w-full sm:w-auto">
                    <div class="relative flex-grow sm:flex-grow-0">
                        <input type="text" x-model.debounce.300ms="searchTerm" @input="fetchCategories" placeholder="Cari kategori..." class="pl-10 pr-4 py-2 w-full border border-slate-300 dark:border-slate-700 rounded-lg focus:ring-primary focus:border-primary dark:bg-slate-800 dark:text-white">
            ...
            <!-- Category List -->
            <div x-html="categoriesHtml" x-show="!hasError">
                {{-- Initial load will be here, subsequent loads via AJAX --}}
                @include('categories._category_list', ['categories' => $categories])
            </div>
        </div>
@endsection